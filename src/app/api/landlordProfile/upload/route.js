...
export async function GET(req) {
  try {
    ...
    const landlordId = decoded.payload.id;

    const landlordKyc = await LandlordKyc.findOne({ landlord: landlordId });

    if (!landlordKyc) {
      return NextResponse.json({ message: "Landlord KYC not found" }, { status: 404 });
    }

    return NextResponse.json(landlordKyc, { status: 200 });
  } catch (error) {
